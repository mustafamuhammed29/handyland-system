export const translations = {
  de: {
    systemTitle: "JansaTech Smart Signage System",
    adminTitle: "HANDYLAND Verwaltungskonsole",
    backToMenu: "Hauptmenü",
    returnToMenuBtn: "Zurück zum Hauptmenü",
    logoutBtn: "Abmelden",
    screen1Title: "Bildschirm 1",
    screen1Sub: "Smartphones & Plakate",
    screen2Title: "Bildschirm 2",
    screen2Sub: "Reparatur (Plakate + Menü)",
    screen3Title: "Bildschirm 3",
    screen3Sub: "Angebote & News Ticker",
    adminBtnTitle: "Verwaltung",
    adminBtnSub: "Plakate, Logo & News verwalten",
    tabDevices: "Bildschirm 1 (Smartphones)",
    tabRepairs: "Bildschirm 2 (Reparatur)",
    tabOffers: "Bildschirm 3 (Angebote)",
    tabSettings: "Einstellungen (Logo, PIN, Timer & Wetter)",
    uploadTitle: "Plakat/Video hochladen für",
    selectImagePrompt: "Klicken Sie hier, um ein Bild oder Video auszuwählen",
    noCropNote: "(Bilder/Videos werden zu 100% ohne Abschneiden angezeigt)",
    changeImage: "Medien ändern",
    uploadBtn: "Medien auf Bildschirm hochladen",
    uploading: "Wird hochgeladen...",
    displayedPosters: "Angezeigte Medien auf",
    noPosters: "Derzeit sind keine Medien für diesen Bildschirm hochgeladen.",
    posterLabel: "Medium",
    deletePosterTooltip: "Medium löschen",
    mediaDimensionsLabel: "Abmessungen:",
    perfect169Badge: "16:9 Perfekt für TV",
    ambientBlurBadge: "Automatischer Weichzeichner aktiv",
    crop169OptionLabel: "Bild automatisch auf 16:9 Vollbild zuschneiden (Ohne Ränder)",
    repairPricesTitle: "Reparatur-Preisliste steuern (Bildschirm 2)",
    deviceModelLabel: "Gerätemodell (z. B. iPhone 13):",
    serviceNameLabel: "Reparaturdienst (z. B. Display Express):",
    priceLabel: "Preis (z. B. 79 €):",
    addPriceBtn: "Neuen Preis hinzufügen",
    displayedPrices: "Aktuelle Reparatur-Preisliste",
    noPrices: "Derzeit sind keine Preise in der Liste.",
    tickerControlTitle: "Lauftext-Steuerung (HANDYLAND News)",
    tickerInstruction: "Dieser Text läuft live unten auf Bildschirm 3 (Angebote):",
    tickerInputLabel: "Neuen Ankündigungstext eingeben:",
    tickerPlaceholder: "Schreiben Sie hier Ihr Angebot oder Ihre Ankündigung...",
    saveTickerBtn: "Neuen Lauftext speichern",
    resetTickerBtn: "Standard-Lauftext wiederherstellen",
    tickerSpeedTitle: "Geschwindigkeit des Lauftextes (in Sekunden)",
    tickerSpeedLabel: "Dauer einer vollständigen Textschleife (z. B. 15 = Schnell, 25 = Normal, 40 = Langsam):",
    saveTickerSpeedBtn: "Lauftext-Geschwindigkeit speichern",
    fontSizeTitle: "Schriftgröße steuern (Textgröße)",
    fontSizeLabel: "Wählen Sie die Anzeige-Schriftgröße für Texte & Preise:",
    saveFontSizeBtn: "Schriftgröße speichern",
    fontNormal: "Normal (100%)",
    fontLarge: "Groß (120%)",
    fontXLarge: "Sehr groß (140%)",
    smokeIntensityTitle: "Shisha-Raucheffekt & Kontrast (Kanka)",
    smokeIntensityDesc: "Steuern Sie die Dichte, Stärke und Sichtbarkeit des Shisha-Rauchs auf den Bildschirmen.",
    smokeIntensityLevel: "Rauchstärke / Kontrast:",
    smokeOff: "Aus (0%)",
    smokeLight: "Leicht (30%)",
    smokeNormal: "Ausgewogen (50%)",
    smokeDense: "Stark (75%)",
    smokeMax: "Maximal (100%)",
    saveSmokeBtn: "Rauchstärke speichern",
    headerSubtitleTitle: "Kopfzeilen-Text steuern (Header Subtitle)",
    headerSubtitleInstruction: "Dieser Text wird oben rechts in der Kopfzeile aller Bildschirme angezeigt:",
    headerSubtitleInputLabel: "Neuen Kopfzeilen-Text eingeben:",
    saveSubtitleBtn: "Kopfzeilen-Text speichern",
    resetSubtitleBtn: "Standard-Kopfzeilen-Text wiederherstellen",
    logoControlTitle: "Geschäftslogo ändern (HANDYLAND Logo)",
    currentLogoLabel: "Aktuelles Geschäftslogo:",
    selectLogoPrompt: "Neues Logo-Bild auswählen",
    saveLogoBtn: "Neues Logo speichern",
    resetLogoBtn: "Standard-Logo wiederherstellen",
    screenTimersTitle: "Timer pro Bildschirm (in Sekunden)",
    screenTimersInstruction: "Geben Sie für jeden Bildschirm die Anzeigedauer pro Plakat selbst ein:",
    timerScreen1Label: "Bildschirm 1 (Smartphones):",
    timerScreen2Label: "Bildschirm 2 (Reparatur):",
    timerScreen3Label: "Bildschirm 3 (Angebote):",
    saveTimersBtn: "Alle Timer speichern",
    screenTitlesTitle: "Bildschirm-Titel anpassen",
    screenTitlesInstruction: "Ändern Sie die Anzeige-Namen der 3 Bildschirme frei nach Wunsch:",
    titleScreen1Label: "Titel von Bildschirm 1:",
    titleScreen2Label: "Titel von Bildschirm 2:",
    titleScreen3Label: "Titel von Bildschirm 3:",
    saveScreenTitlesBtn: "Bildschirm-Titel speichern",
    resetScreenTitlesBtn: "Standard-Titel wiederherstellen",
    confirmResetScreenTitles: "Möchten Sie die Bildschirm-Titel wirklich zurücksetzen?",
    pinProtectionTitle: "PIN-Schutz für Verwaltungskonsole",
    pinInputLabel: "Neuen 4-stelligen PIN-Code eingeben:",
    savePinBtn: "PIN-Code speichern",
    enterPinPrompt: "Bitte geben Sie den PIN-Code ein, um auf die Verwaltungskonsole zuzugreifen:",
    wrongPin: "Falscher PIN-Code! Zugriff verweigert.",
    weatherCityTitle: "Stadt für Live-Wetter-Widget",
    weatherCityLabel: "Stadtname für Live-Temperatur (z. B. Heidelberg, Frankfurt):",
    saveCityBtn: "Stadt speichern",
    confirmResetLogo: "Möchten Sie das Logo wirklich auf das Standard-HANDYLAND-Logo zurücksetzen?",
    confirmResetTicker: "Möchten Sie den Lauftext wirklich auf den Standardtext zurücksetzen?",
    confirmResetSubtitle: "Möchten Sie den Kopfzeilen-Text wirklich auf den Standardtext zurücksetzen?",
    imageTooLarge: "Die Datei ist zu groß. Bitte wählen Sie eine Datei unter 10 MB.",
    logoTooLarge: "Das Logo ist zu groß. Bitte wählen Sie ein Bild unter 2 MB.",
    selectImageFirst: "Bitte wählen Sie zuerst eine Datei aus.",
    uploadSuccess: "Erfolgreich hochgeladen!",
    uploadError: "Fehler beim Hochladen der Datei.",
    saveSuccess: "Erfolgreich gespeichert!",
    resetSuccess: "Erfolgreich zurückgesetzt!",
    offlineNotice: "Offline-Modus aktiv",
    maintenanceTitle: "Wartungsmodus",
    maintenanceDesc: "Die Website wird derzeit gewartet. Bitte haben Sie einen Moment Geduld.",
    enableMaintenanceBtn: "Wartungsmodus aktivieren",
    disableMaintenanceBtn: "Wartungsmodus deaktivieren",
    forceReloadBtn: "Alle Bildschirme aktualisieren (Force Reload)",
    systemControlTitle: "Systemsteuerung (Wartung & Reload)",
    topBarControlTitle: "Oberste Leiste (Uhr & Wetter)",
    topBarControlDesc: "Steuert die Anzeige der oberen Leiste mit Uhrzeit, Live-Wetter und Logo auf den Bildschirmen.",
    topBarVisible: "Leiste anzeigen (Aktiv)",
    topBarHidden: "Leiste ausblenden (Vollbild)",
    topBarToggleBtn: "Status umschalten",
  },
  ar: {
    systemTitle: "نظام JansaTech الشامل لإدارة الشاشات",
    adminTitle: "لوحة تحكم شاشات HANDYLAND",
    backToMenu: "القائمة الرئيسية",
    returnToMenuBtn: "العودة للقائمة",
    logoutBtn: "تسجيل خروج",
    screen1Title: "شاشة 1",
    screen1Sub: "Smartphones (بوسترات وفيديو)",
    screen2Title: "شاشة 2",
    screen2Sub: "Reparatur (بوسترات + القائمة)",
    screen3Title: "شاشة 3",
    screen3Sub: "Angebote (بوسترات العروض)",
    adminBtnTitle: "لوحة التحكم",
    adminBtnSub: "إدارة الشاشات والأخبار والشعار والأسعار",
    tabDevices: "شاشة الأجهزة (1)",
    tabRepairs: "شاشة الصيانة والأسعار (2)",
    tabOffers: "شاشة العروض (3)",
    tabSettings: "الإعدادات (المؤقتات، PIN، الطقس والخط)",
    uploadTitle: "رفع بوستر أو فيديو لـ",
    selectImagePrompt: "اضغط هنا لاختيار صورة بوستر أو مقطع فيديو",
    noCropNote: "(تظهر الصور والفيديوهات بالكامل 100% بدون أي قص)",
    changeImage: "تغيير الملف",
    uploadBtn: "رفع الوسائط للشاشة",
    uploading: "جاري الرفع...",
    displayedPosters: "الوسائط المعروضة على",
    noPosters: "لا توجد بوسترات أو فيديوهات مرفوعة لهذه الشاشة حالياً.",
    posterLabel: "ملف",
    deletePosterTooltip: "حذف الملف",
    mediaDimensionsLabel: "أبعاد الملف:",
    perfect169Badge: "16:9 مثالي ومطابق للتلفزيون",
    ambientBlurBadge: "تعبئة الخلفية بالضباب تلقائياً",
    crop169OptionLabel: "قص وتعبئة الصورة تلقائياً لملء التلفزيون 16:9 بالكامل (بدون حواف)",
    repairPricesTitle: "إدارة قائمة أسعار الصيانة (تظهر في شاشة 2)",
    deviceModelLabel: "موديل الجهاز (مثل iPhone 13):",
    serviceNameLabel: "نوع الخدمة (مثل تبديل شاشة أصلية):",
    priceLabel: "السعر (مثل 79 €):",
    addPriceBtn: "إضافة خدمة وسعر جديد",
    displayedPrices: "قائمة أسعار الصيانة المعروضة",
    noPrices: "لا توجد أسعار مضافة في القائمة حالياً.",
    tickerControlTitle: "التحكم بالنص الإخباري المتحرك (Ticker)",
    tickerInstruction: "هذا النص يتحرك بشكل مباشر وشغّال في أسفل الشاشة 3 (شاشة العروض):",
    tickerInputLabel: "اكتب الخبر أو الإعلان هنا:",
    tickerPlaceholder: "اكتب الخبر باللغة الألمانية أو العربية...",
    saveTickerBtn: "حفظ النص الجديد للأخبار",
    resetTickerBtn: "استعادة النص الإخباري الافتراضي",
    tickerSpeedTitle: "سرعة حركة الشريط الإخباري (بالثواني)",
    tickerSpeedLabel: "مدة الدورة الكاملة للنص (مثلاً: 15 = سريع، 25 = عادي، 40 = بطيء):",
    saveTickerSpeedBtn: "حفظ سرعة الشريط",
    fontSizeTitle: "التحكم بحجم الخط للشاشات",
    fontSizeLabel: "اختر حجم الخط المناسب للأسعار والنصوص على الشاشات:",
    saveFontSizeBtn: "حفظ حجم الخط",
    fontNormal: "عادي (100%)",
    fontLarge: "كبير (120%)",
    fontXLarge: "كبير جداً (140%)",
    smokeIntensityTitle: "التحكم في ظهور وتباين دخان الشيشة (شاشات كانكا)",
    smokeIntensityDesc: "تحكم بقوة وكثافة وتباين ظهور سحب دخان الشيشة على الشاشات أو إيقاف التأثير تماماً.",
    smokeIntensityLevel: "قوة الدخان والتباين:",
    smokeOff: "إيقاف (0%)",
    smokeLight: "خفيف (30%)",
    smokeNormal: "متوازن (50%)",
    smokeDense: "واضح (75%)",
    smokeMax: "كثيف جداً (100%)",
    saveSmokeBtn: "حفظ قوة الدخان",
    headerSubtitleTitle: "التحكم بالنص الفرعي للهيدر العلوي",
    headerSubtitleInstruction: "هذا النص يظهر في أعلى يمين الهيدر بجميع الشاشات:",
    headerSubtitleInputLabel: "اكتب نص الهيدر الفرعي الجديد:",
    saveSubtitleBtn: "حفظ نص الهيدر الجديد",
    resetSubtitleBtn: "استعادة نص الهيدر الافتراضي",
    logoControlTitle: "تغيير شعار المحل (HANDYLAND Logo)",
    currentLogoLabel: "الشعار الحالي المعروض على الشاشات:",
    selectLogoPrompt: "اختيار صورة شعار جديدة",
    saveLogoBtn: "حفظ الشعار الجديد",
    resetLogoBtn: "استعادة الشعار الافتراضي",
    screenTimersTitle: "مؤقت التقليب الخاص لكل شاشة (بالثواني)",
    screenTimersInstruction: "ادخل عدد ثواني عرض البوستر المفضل لكل شاشة بشكل مستقل:",
    timerScreen1Label: "مؤقت شاشة 1 (الأجهزة):",
    timerScreen2Label: "مؤقت شاشة 2 (الصيانة):",
    timerScreen3Label: "مؤقت شاشة 3 (العروض):",
    saveTimersBtn: "حفظ مؤقتات الشاشات",
    screenTitlesTitle: "تغيير أسماء الشاشات الثلاث",
    screenTitlesInstruction: "يمكنك تغيير أسماء وتفاصيل الشاشات بحرية (مثل Hauptmenü / المنيو الرئيسي):",
    titleScreen1Label: "عنوان الشاشة 1:",
    titleScreen2Label: "عنوان الشاشة 2:",
    titleScreen3Label: "عنوان الشاشة 3:",
    saveScreenTitlesBtn: "حفظ أسماء الشاشات",
    resetScreenTitlesBtn: "استعادة الأسماء الافتراضية",
    confirmResetScreenTitles: "هل أنت تأكد من استعادة أسماء الشاشات الافتراضية؟",
    pinProtectionTitle: "حماية لوحة التحكم برمز سر (PIN)",
    pinInputLabel: "اكتب رمز PIN الجديد (من 4 أرقام):",
    savePinBtn: "حفظ رمز PIN",
    enterPinPrompt: "يرجى كتابة رمز PIN للدخول إلى لوحة التحكم:",
    wrongPin: "رمز PIN غير صحيح! تم رفض الدخول.",
    weatherCityTitle: "المدينة لودجت الطقس المباشر",
    weatherCityLabel: "اسم المدينة لجلب الحرارة الحقيقية (مثل هايدلبرغ Heidelberg):",
    saveCityBtn: "حفظ المدينة",
    confirmResetLogo: "هل أنت تأكد من إرجاع الشعار إلى الشعار الأساسي الافتراضي لـ HANDYLAND؟",
    confirmResetTicker: "هل أنت تأكد من استعادة نص الشريط الإخباري الافتراضي؟",
    confirmResetSubtitle: "هل أنت تأكد من استعادة نص الهيدر الافتراضي؟",
    imageTooLarge: "حجم الملف كبير جداً. يرجى اختيار ملف أقل من 10 ميجابايت.",
    logoTooLarge: "حجم الشعار كبير جداً. يرجى اختيار صورة أقل من 2 ميجابايت.",
    selectImageFirst: "يرجى اختيار ملف أولاً.",
    uploadSuccess: "تم الرفع بنجاح!",
    uploadError: "حدث خطأ أثناء رفع الملف.",
    saveSuccess: "تم الحفظ بنجاح!",
    resetSuccess: "تمت الاستعادة بنجاح!",
    offlineNotice: "وضع الأوفلاين نشط",
    maintenanceTitle: "وضع الصيانة",
    maintenanceDesc: "الموقع يخضع للصيانة والتحديث حالياً. يرجى الانتظار قليلاً.",
    enableMaintenanceBtn: "تفعيل وضع الصيانة",
    disableMaintenanceBtn: "إيقاف وضع الصيانة",
    forceReloadBtn: "إجبار جميع الشاشات على التحديث (إعادة تحميل)",
    systemControlTitle: "التحكم بالنظام (الصيانة والتحديث)",
    topBarControlTitle: "الشريط العلوي (شريط الساعة والطقس)",
    topBarControlDesc: "التحكم في إظهار أو إخفاء الشريط العلوي بالكامل (الساعة، الطقس المباشر، الشعار) على شاشات التلفزيون.",
    topBarVisible: "الشريط ظاهر (مفعّل)",
    topBarHidden: "الشريط مخفي (ملء الشاشة)",
    topBarToggleBtn: "تبديل حالة الشريط",
  }
};
